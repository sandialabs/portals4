...
AC_DEFUN([SANDIA_CHECK_WORKING_VALLOC],
[AC_CACHE_CHECK([if valloc()'d memory can be free()'d],
  [sandia_cv_working_valloc],
  [AC_TRY_RUN([
#include <stdio.h>
#include <stdlib.h>
#if HAVE_UNISTD_H
...
int main()
{
  ...
  exit (0);
     ...
     [sandia_cv_working_valloc=yes],
     [sandia_cv_working_valloc=no],
     [sandia_cv_working_valloc=no])
  ])
AS_IF([test "x$sandia_cv_working_valloc" = xyes],
