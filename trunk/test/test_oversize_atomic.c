         ...
         char *argv[])
{
    ...
    readval = malloc(sizeof(double) * BUFSIZE);
    ...
    read_md.length = BUFSIZE * sizeof(double);
    ...
    /* twiddle rank 0's value */
    {
        ...
        CHECK_RETURNVAL(PtlAtomic(read_md_handle, 0, BUFSIZE*sizeof(double),
                                  PTL_OC_ACK_REQ, r0, logical_pt_index, 1, 0,
                                  NULL, 0, PTL_SUM, PTL_DOUBLE));
    ...
    tmp = readval[0];
    for (unsigned idx=0; idx<BUFSIZE; idx++) {
        if (!fpequal(readval[idx], 1.1)) {
            fprintf(stderr, "bad value at idx %u (readval[%u] = %g)\n",
                            idx, idx, readval[idx]);
            abort();
        }
        // check that all entries are identical to one another
        if (!fpequal(tmp, readval[idx])) {
            fprintf(stderr, "bad value at idx %u (readval[%u] = %g), "
                            "expected %g everywhere (are atomics atomic?)\n",
    ...
    if (myself.rank == 0) {
        ...
        for (unsigned idx=0; idx<BUFSIZE; idx++) {
            //printf("value[%u] = %g\n", idx, value[idx]);
            if (!fpequal(value[idx], 77.5 + (1.1 * num_procs))) {
            //if (!fpequal(value[idx], 1.5 * (double)idx + (1.1 * num_procs))) {
                fprintf(stderr, "bad result value at idx %u (value[%u] = %g)"
                        ", expected %g\n",
                        idx, idx, value[idx], 77.5+(1.1*num_procs));
                failed = 1;
            }
            // check that all entries are identical to one another
            if (!fpequal(tmp, value[idx])) {
                fprintf(stderr,
                        "bad result value at idx %u (value[%u] = %g), "
                        "expected %g everywhere (are atomics atomic?)\n",
