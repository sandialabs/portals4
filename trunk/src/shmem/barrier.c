...
void API_FUNC runtime_barrier(void)
{
    ...
    if (barrier_inited == 0) {
        ...
        COLLECTOR.phys.nid = strtol(getenv("PORTALS4_COLLECTOR_NID"), NULL, 10);
        COLLECTOR.phys.pid = strtol(getenv("PORTALS4_COLLECTOR_PID"), NULL, 10);
    }
    ...
    assert(PtlPut(mdh, 0, 0, PTL_CT_ACK_REQ, COLLECTOR, 0, 0, 0, NULL, 0) == PTL_OK);
