                           ...
                           mr_t *local_mr, ptl_size_t len)
{
    ...
    if (dir == DATA_DIR_IN)
        copied =
            knem_copy(ni, remote_iovec->cookie, remote_iovec->offset,
                      local_mr->knem_cookie, local_addr - local_mr->addr,
                      len);
    else
        copied =
            knem_copy(ni, local_mr->knem_cookie, local_addr - local_mr->addr,
...
static inline void advance_remote_addr(struct mem_iovec *remote_iovec,
                                       ptl_size_t len)
{
#if (WITH_TRANSPORT_SHMEM && USE_KNEM)
...
static ptl_size_t do_mem_copy(buf_t *buf, ptl_size_t rem_len,
                              struct mem_iovec *iovec, ptl_size_t *loc_index,
                              ...
                              data_dir_t dir)
{
    ...
    while (tot_len < rem_len) {
        ...
        if (me->num_iov) {
            ...
            iov =
                ((ptl_iovec_t *)addr_to_ppe(me->start, me->mr_start)) +
                *loc_index;
...
int mem_do_transfer(buf_t *buf)
{
    ...
    ptl_size_t *resid =
        (dir == DATA_DIR_IN) ? &buf->put_resid : &buf->get_resid;
    ...
    while (*resid > 0) {
        ...
        bytes =
            do_mem_copy(buf, iovec.length, &iovec, &iov_index, &iov_off,
                        buf->le->num_iov, dir);
        if (!bytes)
            return PTL_FAIL;
    ...
    for (i = 0; i < map_size; i++) {
        if (mapping[i].phys.nid == iface->id.phys.nid) {
            ...
            if (get_param(PTL_ENABLE_MEM)) {
                ...
                conn->shmem.local_rank = ni->mem.node_size;
#else
#error
#endif
        ...
        ni->mem.hash =
            crc32((unsigned char *)&mapping[i].phys, ni->mem.hash,
                  sizeof(mapping[i].phys));
    }
