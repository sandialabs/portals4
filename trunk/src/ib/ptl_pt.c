 ...
 */
static int get_pt_index(ni_t *ni, ptl_pt_index_t req, ptl_pt_index_t *index_p)
{
...
int _PtlPTFree(PPEGBL ptl_handle_ni_t ni_handle, ptl_pt_index_t pt_index)
{
    ...
    if (unlikely
        (pt_index > ni->limits.max_pt_index || !ni->pt[pt_index].in_use)) {
        pthread_mutex_unlock(&ni->pt_mutex);
        err = PTL_ARG_INVALID;
        goto err2;
    }
    ...
    if (!list_empty(&pt->priority_list) || !list_empty(&pt->overflow_list)) {
        pthread_mutex_unlock(&ni->pt_mutex);
        err = PTL_PT_IN_USE;
        goto err2;
    }
