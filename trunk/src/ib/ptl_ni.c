...
static void set_limits(ni_t *ni, const ptl_ni_limits_t *desired)
{
    if (desired) {
        ni->limits.max_entries =
            chk_param(PTL_LIM_MAX_ENTRIES, desired->max_entries);
        ni->limits.max_unexpected_headers =
            chk_param(PTL_LIM_MAX_UNEXPECTED_HEADERS,
                      desired->max_unexpected_headers);
        ni->limits.max_mds = chk_param(PTL_LIM_MAX_MDS, desired->max_mds);
        ni->limits.max_cts = chk_param(PTL_LIM_MAX_CTS, desired->max_cts);
        ni->limits.max_eqs = chk_param(PTL_LIM_MAX_EQS, desired->max_eqs);
        ni->limits.max_pt_index =
            chk_param(PTL_LIM_MAX_PT_INDEX, desired->max_pt_index);
        ni->limits.max_iovecs =
            chk_param(PTL_LIM_MAX_IOVECS, desired->max_iovecs);
        ni->limits.max_list_size =
            chk_param(PTL_LIM_MAX_LIST_SIZE, desired->max_list_size);
        ni->limits.max_triggered_ops =
            chk_param(PTL_LIM_MAX_TRIGGERED_OPS, desired->max_triggered_ops);
        ni->limits.max_msg_size =
            chk_param(PTL_LIM_MAX_MSG_SIZE, desired->max_msg_size);
        ni->limits.max_atomic_size =
            chk_param(PTL_LIM_MAX_ATOMIC_SIZE, desired->max_atomic_size);
        ...
        ni->limits.max_volatile_size =
            chk_param(PTL_LIM_MAX_VOLATILE_SIZE, desired->max_volatile_size);
        ni->limits.features = chk_param(PTL_LIM_FEATURES, desired->features);
        if (desired->features & PTL_TARGET_BIND_INACCESSIBLE)
            ni->limits.features |= PTL_TARGET_BIND_INACCESSIBLE;
    } else {
        ni->limits.max_entries = get_param(PTL_LIM_MAX_ENTRIES);
        ni->limits.max_unexpected_headers =
            get_param(PTL_LIM_MAX_UNEXPECTED_HEADERS);
        ni->limits.max_mds = get_param(PTL_LIM_MAX_MDS);
        ni->limits.max_cts = get_param(PTL_LIM_MAX_CTS);
        ni->limits.max_eqs = get_param(PTL_LIM_MAX_EQS);
        ni->limits.max_pt_index = get_param(PTL_LIM_MAX_PT_INDEX);
        ni->limits.max_iovecs = get_param(PTL_LIM_MAX_IOVECS);
        ni->limits.max_list_size = get_param(PTL_LIM_MAX_LIST_SIZE);
        ni->limits.max_triggered_ops = get_param(PTL_LIM_MAX_TRIGGERED_OPS);
        ni->limits.max_msg_size = get_param(PTL_LIM_MAX_MSG_SIZE);
        ni->limits.max_atomic_size = get_param(PTL_LIM_MAX_ATOMIC_SIZE);
        ni->limits.max_fetch_atomic_size =
        ...
        ni->limits.max_volatile_size = get_param(PTL_LIM_MAX_VOLATILE_SIZE);
...
static int init_pools(ni_t *ni)
{
    ...
    err =
        pool_init(gbl, &ni->mr_pool, "mr", sizeof(mr_t), POOL_MR,
                  (obj_t *)ni);
    if (err) {
        WARN();
        return err;
    ...
    err =
        pool_init(gbl, &ni->md_pool, "md", sizeof(md_t), POOL_MD,
                  (obj_t *)ni);
    if (err) {
        WARN();
        return err;
    ...
    if (ni->options & PTL_NI_MATCHING) {
        ...
        err =
            pool_init(gbl, &ni->me_pool, "me", sizeof(me_t), POOL_ME,
                      (obj_t *)ni);
        if (err) {
            WARN();
            return err;
        ...
        err =
            pool_init(gbl, &ni->le_pool, "le", sizeof(le_t), POOL_LE,
                      (obj_t *)ni);
        if (err) {
            WARN();
            return err;
    ...
    err =
        pool_init(gbl, &ni->eq_pool, "eq", sizeof(eq_t), POOL_EQ,
                  (obj_t *)ni);
    if (err) {
        WARN();
        return err;
    ...
    err =
        pool_init(gbl, &ni->ct_pool, "ct", sizeof(ct_t), POOL_CT,
                  (obj_t *)ni);
    if (err) {
        WARN();
        return err;
    ...
    err =
        pool_init(gbl, &ni->buf_pool, "buf", real_buf_t_size(), POOL_BUF,
                  (obj_t *)ni);
    if (err) {
        WARN();
        return err;
    ...
    err =
        pool_init(gbl, &ni->conn_pool, "conn", sizeof(conn_t), POOL_BUF,
                  (obj_t *)ni);
    if (err) {
        WARN();
        return err;
...
//              conn->sin.sin_port = entry->pid;
        ptl_info("entry: %i, ADDR: %s PID: %i \n", i,
                 inet_ntoa(conn->sin.sin_addr), (entry->pid));
#endif
    }
               ...
               ptl_process_t *mapping, ptl_size_t *actual_map_size)
{
    ...
    if (map_size)
        memcpy(mapping, ni->logical.mapping,
