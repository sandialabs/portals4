...
static void PtlInternalAnnounceLEDelivery(const ptl_handle_eq_t                 eq_handle,
                                          ...
                                          ptl_internal_header_t *const restrict hdr);
#endif
static void PtlInternalAnnounceLELink(ptl_ppe_ni_t *, 
...
int 
_PtlLEAppend( ptl_ppe_ni_t *ppe_ni, 
...
//    ptl_internal_handle_converter_t       leh    = { .s.selector = HANDLE_LE_CODE };
    ptl_internal_handle_converter_t       le_hc    = (ptl_internal_handle_converter_t) le_h;
...
#endif
    ppe_ni->ppe_le[le_hc.s.code].visible = *le;
    ppe_ni->ppe_le[le_hc.s.code].pt_index = pt_index;
    ppe_ni->ppe_le[le_hc.s.code].ptl_list = ptl_list;
...
//                        PtlInternalDeallocUnexpectedHeader(tmp);
                    ...
                    if ((le->options & PTL_LE_USE_ONCE) == 0) {
                        ...
                        if ((tEQ != PTL_EQ_NONE) ||
                            (le->ct_handle != PTL_CT_NONE)) {
                            __sync_synchronize();
                            PtlInternalAnnounceLEDelivery(NULL,tEQ,
                                                          le->ct_handle,
                                                          le->options,
                                                          mlength, (uintptr_t)
                                                          cur->buffered_data,
...
//                        PtlInternalDeallocUnexpectedHeader(cur);
            ...
            }
            {
                ...
                PtlInternalAnnounceLELink(ppe_ni, tEQ, options, pt_index, user_ptr);
            }
            break;
...
{                                      /*{{{ */
    ...
    ptl_table_entry_t *restrict const     t = 
                    &(ppe_ni->ppe_pt[ppe_ni->ppe_le[le.s.code].pt_index]);
...
//        &(les[le.s.ni][le.s.code].Qentry);
    const ptl_internal_appendLE_t *restrict const dq_target =
        &(ppe_ni->ppe_le[le.s.code].Qentry);
    ...
    if ( ppe_ni->ppe_le[le.s.code].ptl_list == PTL_PRIORITY_LIST) {
        ...
        if (dq == &(ppe_ni->ppe_le[le.s.code].Qentry)) {
            if (dq->next != NULL) {
            ...
            while (dq != &(ppe_ni->ppe_le[le.s.code].Qentry) && dq != NULL) {
                prev = dq;
                dq   = dq->next;
            }
            if (dq == NULL) {
...
ptl_pid_t INTERNAL PtlInternalLEDeliver( nal_ctx_t *nal_ctx, 
...
{                                      /*{{{ */
    ...
    if (entry != NULL) {
        ...
        *********************************************************/
        ...
        nal_ctx->u.le.ppe_le = (ptl_ppe_le_t*) entry;
        switch (hdr->type & HDR_TYPE_BASICMASK) {
            case HDR_TYPE_PUT:
            case HDR_TYPE_ATOMIC:
            case HDR_TYPE_FETCHATOMIC:
...
# endif
                ...
                PtlInternalPerformDelivery(nal_ctx, hdr->type, effective_start,
                                           0,//hdr->data, 
                                            fragment_mlength,
                                           hdr);
#endif          /* ifdef USE_TRANSFER_ENGINE */
            ...
            __sync_synchronize();
            PtlInternalAnnounceLEDelivery( nal_ctx, tEQ,
                                          le.ct_handle,
                                          le.options,
                                          msg_mlength,
                                          (uintptr_t)report_this_start,
...
void
PtlInternalAnnounceLEDelivery( nal_ctx_t *nal_ctx,
                                         const ptl_handle_eq_t                 eq_handle,
                                          const ptl_handle_ct_t                 ct_handle,
                                          const unsigned int                    options,
                                          const uint_fast64_t                   mlength,
...
{                                      /*{{{ */
    ...
    if (eq_handle != PTL_EQ_NONE) {
        if (((foundin == OVERFLOW) && ((options & PTL_LE_EVENT_OVER_DISABLE) == 0)) ||
            ((foundin == PRIORITY) && ((options & (PTL_LE_EVENT_COMM_DISABLE | PTL_LE_EVENT_SUCCESS_DISABLE)) == 0))) {
            ptl_internal_event_t e;
            PTL_INTERNAL_INIT_TEVENT(e, hdr, user_ptr);
            if (foundin == OVERFLOW) {
                switch (e.type & HDR_TYPE_BASICMASK) {
                    case HDR_TYPE_PUT:
                        e.type = PTL_EVENT_PUT_OVERFLOW;
                        break;
                    case HDR_TYPE_ATOMIC:
...
static void PtlInternalAnnounceLELink(ptl_ppe_ni_t *ppe_ni,
                                    const ptl_handle_eq_t eq_handle,
                                      const unsigned int    options,
                                      const ptl_pt_index_t  pt_index,
                                      void *const           user_ptr)
