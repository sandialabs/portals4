...
static void *
ptl_mmap(void *start, size_t length, int prot, int flags, int fd, off_t offset)
{
  void *tmp = mmap(start, length, prot, flags, fd, offset);
  if (NULL != tmp) {
    alloc_add_mem(tmp, length);
  }
  return tmp;
}

static int
ptl_munmap(void *start, size_t length)
{
  alloc_remove_mem(start, length);
...
#define MORECORE ptl_morecore
#define MMAP(addr, size, prot, flags) \
  (ptl_mmap((addr), (size), (prot), (flags)|MAP_ANONYMOUS, -1, 0))
#define munmap(a,b) ptl_munmap(a,b)
...
#endif
#endif /* Portals Change */

/* Portals change: we don't want any of these -- we want to use the
