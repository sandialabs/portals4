...
# define PTL_LOCK_INIT(x) pthread_spin_init(&(x), PTHREAD_PROCESS_SHARED)
# define PTL_LOCK_LOCK(x) pthread_spin_lock(&(x))
# define PTL_LOCK_UNLOCK(x) pthread_spin_unlock(&(x))
#else
# include <pthread.h>
# define PTL_LOCK_TYPE pthread_mutex_t
# define PTL_LOCK_INIT(x) pthread_mutex_init(&(x), NULL)
# define PTL_LOCK_LOCK(x) pthread_mutex_lock(&(x))
# define PTL_LOCK_UNLOCK(x) pthread_mutex_unlock(&(x))
#endif
