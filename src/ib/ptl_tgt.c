 ...
 */
static int check_match(buf_t *buf, const me_t *me)
{
 ...
 */
static int check_perm(buf_t *buf, const le_t *le)
{
	int ret = PTL_NI_OK;
	const req_hdr_t *hdr = (req_hdr_t *)buf->data;
				...
				struct list_head *buf_list)
{
	...
	list_for_each_entry_safe(buf, n, &pt->unexpected_list,
		...
		if ((no_matching || check_match(buf, (me_t *)le)) &&
		    (dont_check_perm || !check_perm(buf, le))) {
...
int check_overflow(le_t *le)
{
	...
	match_le_unexpected(le, 1, &buf_list);
...
/* Check whether that LE/ME matches one or more XT on the unexpected
 * list.
 * TODO copying existing implementation but not sure why we do anything
 * if COMM_DISABLE is set.
 */
...
int check_overflow_search_delete(le_t *le)
{
	...
	match_le_unexpected(le, 0, &buf_list);
