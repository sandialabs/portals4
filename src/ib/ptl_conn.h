...
struct transport {
     ...
     * KNEM, ...). */
    int (*init_prepare_transfer) (struct md * md, data_dir_t dir,
 ...
 * @return < 0 if a < b
 */
static inline int compare_id(const ptl_process_t *id1,
                             const ptl_process_t *id2)
{
    return (id1->phys.nid !=
            id2->phys.nid) ? (id1->phys.nid -
                              id2->phys.nid) : (id1->phys.pid -
                                                id2->phys.pid);
}
