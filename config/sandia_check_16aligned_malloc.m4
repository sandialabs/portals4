...
AC_DEFUN([SANDIA_CHECK_16ALIGNED_MALLOC],
[AC_CACHE_CHECK([if mallocs guarantee 16-byte alignment],
  [sandia_cv_func_malloc_16aligned],
  [AC_TRY_RUN([
#include <stdio.h>
#include <stdlib.h>
#if HAVE_UNISTD_H
...
int main()
{
  ...
  exit (0);
     ...
     [sandia_cv_func_malloc_16aligned=yes],
     [sandia_cv_func_malloc_16aligned=no],
     [sandia_cv_func_malloc_16aligned=no])
  ])
if test "$sandia_cv_func_malloc_16aligned" = yes ; then
...
AC_DEFUN([SANDIA_CHECK_16ALIGNED_CALLOC],
[AC_CACHE_CHECK([if callocs guarantee 16-byte alignment],
  [sandia_cv_func_calloc_16aligned],
  [AC_TRY_RUN([
#include <stdio.h>
#include <stdlib.h>
#if HAVE_UNISTD_H
...
int main()
{
  ...
  exit (0);
     ...
     [sandia_cv_func_calloc_16aligned=yes],
     [sandia_cv_func_calloc_16aligned=no],
     [sandia_cv_func_calloc_16aligned=no])
  ])
if test "$sandia_cv_func_calloc_16aligned" = yes ; then
