...
void* do_work(void* arg) {
    unsigned int which;
    unsigned polltime_ms = 10000;
    struct thread_data *data = (struct thread_data*) arg; 
    printf("I'm a worker thread calling PtlEQPoll\n");
    
    CHECK_RETURNVAL(PtlEQPoll(&data->eq_h, 1, polltime_ms, &data->event, &which));
}
...
int main(int   argc, char *argv[])
{
    ...
    CHECK_RETURNVAL(PtlEQAlloc(ni_h, 8192, &tdata.eq_h));
    CHECK_RETURNVAL(PtlPTAlloc(ni_h, 0, tdata.eq_h, PTL_PT_ANY,
                               &pt_index));
    assert(pt_index == 0);
    ...
    if (rank == 0) {
        ...
        int arg = 0;
        int ret;
        ret = pthread_create(&worker, NULL, do_work, &tdata);
        sleep(1);
        PtlAbort();

        // TODO get return val from EQPoll
        ret = pthread_join(worker, NULL);

        // TODO check return val from EQPoll to make sure PTL_ABORTED was called
    }
