...
int main(int   argc, char *argv[])
{
    ...
    if (rank == 0) {
        ...
        ret = pthread_create(&worker0, NULL, thread_CTPoll, &tdata0);
        ret = pthread_create(&worker1, NULL, thread_CTPoll, &tdata1);
        sleep(1);
        PtlAbort();
        ...
        err = PtlCTPoll(&ct_h, &tests, 1, timeout, &counter, &which);
