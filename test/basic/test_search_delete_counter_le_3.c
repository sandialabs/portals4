// This test confirms that when a match is found and USE_ONCE is set, 
// failure is not incremented. It also confirms that only one 
// match is found, consistent with USE_ONCE.
         ...
         char *argv[])
{
    ...
    if (1 == rank) {
        ...
        CHECK_RETURNVAL(APPEND(ni_h, 0, &value_e, PTL_OVERFLOW_LIST, NULL, &value_e_handle));
        value = 0;
    } else if (0 == rank) {
    ...
    if (1 == rank) {
        ...
        value_e.length = sizeof(uint64_t);
        value_e.uid    = PTL_UID_ANY;
        value_e.options = SSOPTIONS; /* USE_ONCE set */
