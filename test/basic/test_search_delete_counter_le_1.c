// This test just confirms if there is no match doing an LE search on the unexpected list, counter 
// failure gets incremented by one for both USE_ONCE and not USE_ONCE
...
# define SOPTIONS  (PTL_LE_OP_PUT | PTL_LE_EVENT_LINK_DISABLE | PTL_LE_EVENT_CT_OVERFLOW)
# define SSOPTIONS  (PTL_LE_OP_PUT | PTL_LE_EVENT_LINK_DISABLE | PTL_LE_USE_ONCE | PTL_LE_EVENT_CT_OVERFLOW)
         ...
         char *argv[])
{
    ...
    /* 0 writes unexpecteds to 1; in this case nothing is put because we want the LE search to fail.*/
    if (0 == rank) {
        /* write to rank 1 */
        ptl_process_t peer;
	      peer.rank = 1;
...
//        assert(ctc.failure == 0);
    ...
    if (1 == rank) {
        ...
        if (ct_event.success != 0) {
          fprintf(stderr, "When searching for unexpected headers expected 0 but found %d\n", ct_event.success);
          error_found = 1;
        }
        ...
        fprintf(stderr, "ct.failure       : %d\n", ct_event.failure);
        if (ct_event.success != 0) {
          fprintf(stderr, "When searching for unexpected headers with USE_ONCE set, expected 0 but found %d\n", ct_event.success);
          error_found = 1;
        }
        if (ct_event.failure != 1) {
          fprintf(stderr, "When searching for unexpected headers with USE_ONCE set, expected failure = 1, but found %d\n", ct_event.failure);
          error_found = 1;
        }
        if (error_found) {
