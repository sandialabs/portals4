    ...
    char *argv[])
{
    ...
    {
	uint64_t value = myself.rank + 0xdeadbeef, readval;
	ptl_le_t value_le;
	...
	/* set up the LE so that others can read my value */
	...
	/* read rank 0's value */
	target.rank = 0;
	CHECK_RETURNVAL(PtlGet(read_md_handle, 0, sizeof(uint64_t), target, logical_pt_index, 0, NULL, 0));
	noFailures(read_md.ct_handle, 1);
